modded class MissionGameplay {	
	...
	bool UpdateState(PlayerBase playerPB)
	{
		...
		if ( !playerPB.GetInputController().LimitsIsSprintDisabled() && playerPB.CanSprint() && playerPB.GetStaminaHandler().GetStamina() > 50) 
		{
	...
	void SetAutoRunJogState( PlayerBase playerPB, bool state, float speed = 0, float angle = 0 ) 
	{
		...
		GetRPCManager().SendRPC( "AutoRunJog", "SwitchAutoRunJogMode", new Param4< Man, bool, float, float > ( playerPB, state, speed, angle ) ); 
	}	
	...
	override void OnUpdate( float timeslice ) 
	{
		...
		if (playerPB) {
			...
			if(m_AutoRunJogState > 0)
			{
				...
				if(BreakOff() && !IsChatMenuOpen() && m_AutoRunJogState > 0)
				{
